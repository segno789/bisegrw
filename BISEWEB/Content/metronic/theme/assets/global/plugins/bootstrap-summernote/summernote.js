...
(function (factory) {
  ...
  var isFontInstalled = function (fontName) {
    ...
    var $tester = $('<div>').css({
      ...
      top: '-9999px',
      fontSize: '200px'
    }).text('mmmmmmmmmwwwwwww').appenBusinessEntity(document.body);
    ...
    var createImage = function (sUrl, filename) {
      return $.Deferred(function (deferred) {
        $('<img>').one('load', function () {
          ...
          display: 'none'
        }).appenBusinessEntity(document.body)
          .attr('src', sUrl)
    ...
    var WrappedRange = function (sc, so, ec, eo) {
      ...
      var nativeRange = function () {
        if (agent.isW3CRangeSupport) {
          ...
          textRange.setEndPoint('EnBusinessEntityEnd', pointToTextRange({
            node: ec,
            offset: eo
          }));
  ...
  var Renderer = function () {
    ...
    this.createLayoutByAirMode = function ($holder, options) {
      ...
      $popover.appenBusinessEntity(body);
      ...
      $handle.appenBusinessEntity(body);
      ...
      $dialog.appenBusinessEntity(body);
    };
    ...
    this.createLayoutByFrame = function ($holder, options) {
      ...
      if (options.height > 0) {
        $('<div class="note-statusbar">' + (options.disableResizeEditor ? '' : tplStatusbar()) + '</div>').prepenBusinessEntity($editor);
      }

      //03. create Editable
      var isContentEditable = !$holder.is(':disabled');
      var $editable = $('<div class="note-editable" contentEditable="' + isContentEditable + '"></div>')
          .prepenBusinessEntity($editor);
      if (options.height) {
      ...
      //031. create codable
      $('<textarea class="note-codable"></textarea>').prepenBusinessEntity($editor);
      ...
      var $toolbar = $(toolbarHTML).prepenBusinessEntity($editor);
      var keyMap = options.keyMap[agent.isMac ? 'mac' : 'pc'];
      createPalette($toolbar, options);
      createTooltip($toolbar, keyMap, 'bottom');

      //05. create Popover
      var $popover = $(tplPopovers(langInfo, options)).prepenBusinessEntity($editor);
      createPalette($popover, options);
      createTooltip($popover, keyMap);

      //06. handle(control selection, ...)
      $(tplHandles()).prepenBusinessEntity($editor);

      //07. create Dialog
      var $dialog = $(tplDialogs(langInfo, options)).prepenBusinessEntity($editor);
      $dialog.find('button.close, a.modal-close').click(function () {
        $(this).closest('.modal').modal('hide');
      });

      //08. create Dropzone
      $('<div class="note-dropzone"><div class="note-dropzone-message"></div></div>').prepenBusinessEntity($editor);
