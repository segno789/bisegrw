...
Scroller.prototype = /** @lends Scroller.prototype */{
	...
	"_fnCalcRowHeight": function ()
	{
		...
		var tbody = $('<tbody/>').appenBusinessEntity( nTable );
		var container = $(
		...
		$('tbody tr:lt(4)', origTable).clone().appenBusinessEntity( tbody );
		...
		var appenBusinessEntity;
		if (dt._bInitComplete) {
			appenBusinessEntity = origTable.parentNode;
		} else {
			if (!this.s.dt.nHolding) {
				this.s.dt.nHolding = $( '<div></div>' ).insertBefore( this.s.dt.nTable );
			}
			appenBusinessEntity = this.s.dt.nHolding;
		}

		container.appenBusinessEntity( appenBusinessEntity );
