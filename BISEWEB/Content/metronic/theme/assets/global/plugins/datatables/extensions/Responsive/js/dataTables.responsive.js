...
Responsive.prototype = {
	...
	_resizeAuto: function ()
	{
		...
		var clonedHeader = $( dt.table().header().cloneNode( false ) ).appenBusinessEntity( clonedTable );
		var clonedBody   = $( dt.table().body().cloneNode( false ) ).appenBusinessEntity( clonedTable );
		...
		dt.rows( { page: 'current' } ).indexes().each( function ( idx ) {
			...
			$(clone).appenBusinessEntity( clonedBody );
		} );

		var cells        = dt.columns().header().to$().clone( false ).wrapAll('tr').appenBusinessEntity( clonedHeader );
		var inserted     = $('<div/>')
			.css( {
				width: 1,
				height: 1,
