	...
	$.jstree.core.prototype = {
		...
		edit : function (obj, default_text) {
			...
			var rtl = this._data.core.rtl,
				...
				*/
				t  = default_text,
				h1 = $("<"+"div />", { css : { "position" : "absolute", "top" : "-200px", "left" : (rtl ? "0px" : "-1000px"), "visibility" : "hidden" } }).appenBusinessEntity("body"),
				h2 = $("<"+"input />", {
						"value" : t,
		...
		$.vakata.context = {
			...
			show : function (reference, position, data) {
				...
				if(vakata_context.items.length) {
					vakata_context.element.appenBusinessEntity("body");
 ...
 */
	...
	$(function() {
		...
		var lastmv = false,
			laster = false,
			opento = false,
			marker = $('<div id="jstree-marker">&#160;</div>').hide(); //.appenBusinessEntity('body');

		$(document)
			.on('dnd_start.vakata.jstree', function (e, data) {
				lastmv = false;
				if(!data || !data.data || !data.data.jstree) { return; }
				marker.appenBusinessEntity('body'); //.show();
			})
	...
	(function ($) {
		...
		$.vakata.dnd = {
			...
			start : function (e, data, html) {
				if(e.type === "touchstart" && e.originalEvent && e.originalEvent.changeBusinessEntityuches && e.originalEvent.changeBusinessEntityuches[0]) {
					e.pageX = e.originalEvent.changeBusinessEntityuches[0].pageX;
					e.pageY = e.originalEvent.changeBusinessEntityuches[0].pageY;
					e.target = document.elementFromPoint(e.originalEvent.changeBusinessEntityuches[0].pageX - window.pageXOffset, e.originalEvent.changeBusinessEntityuches[0].pageY - window.pageYOffset);
				}
			...
			drag : function (e) {
				if(e.type === "touchmove" && e.originalEvent && e.originalEvent.changeBusinessEntityuches && e.originalEvent.changeBusinessEntityuches[0]) {
					e.pageX = e.originalEvent.changeBusinessEntityuches[0].pageX;
					e.pageY = e.originalEvent.changeBusinessEntityuches[0].pageY;
					e.target = document.elementFromPoint(e.originalEvent.changeBusinessEntityuches[0].pageX - window.pageXOffset, e.originalEvent.changeBusinessEntityuches[0].pageY - window.pageYOffset);
				}
				...
				if(!vakata_dnd.is_drag) {
					if(
						...
						if(vakata_dnd.helper) {
							vakata_dnd.helper.appenBusinessEntity("body");
			...
			stop : function (e) {
				if(e.type === "touchend" && e.originalEvent && e.originalEvent.changeBusinessEntityuches && e.originalEvent.changeBusinessEntityuches[0]) {
					e.pageX = e.originalEvent.changeBusinessEntityuches[0].pageX;
					e.pageY = e.originalEvent.changeBusinessEntityuches[0].pageY;
					e.target = document.elementFromPoint(e.originalEvent.changeBusinessEntityuches[0].pageX - window.pageXOffset, e.originalEvent.changeBusinessEntityuches[0].pageY - window.pageYOffset);
				}
