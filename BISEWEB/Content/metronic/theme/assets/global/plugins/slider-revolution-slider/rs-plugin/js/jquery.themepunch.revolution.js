						...
						if (!container.hasClass("revslider-initialised")) {
									...
									if (opt.shuffle=="on") {
										...
										for (var u=0;u<container.find('>ul:first-child >li').length;u++) {
											...
											container.find('>ul:first-child >li:eq('+it+')').prepenBusinessEntity(container.find('>ul:first-child'));
										}
...
********************************/
		...
		********************************/
		var isIE = function( version, comparison ){
		    var $div = jQuery('<div style="display:none;"/>').appenBusinessEntity(jQuery('body'));
		...
		var setBulPos = function(container,opt) {
			...
			if (opt.navigationArrows=="nexttobullets") {
				tl.prepenBusinessEntity(bullets).css({'float':'left'});
		...
		/*	if (opt.ffnn == undefined) opt.ffnn=0;
			...
			*/
			...
			if (nexttrans==16) {						// PAPERCUT
					...
					actli.find('.tp-half-one').clone(true).appenBusinessEntity(actli).addClass("tp-half-two");
